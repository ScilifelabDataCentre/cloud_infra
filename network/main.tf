...
resource "openstack_networking_subnet_v2" "created" {
  ...
  network_id = "${openstack_networking_network_v2.created[count.index].id}"
...
resource "openstack_networking_router_interface_v2" "created" {
  # create only if not specified in var.network_name
  count     = "${var.network_name == "" ? 1 : 0}"
  router_id = "${openstack_networking_router_v2.created[count.index].id}"
  subnet_id = "${openstack_networking_subnet_v2.created[count.index].id}"
}
